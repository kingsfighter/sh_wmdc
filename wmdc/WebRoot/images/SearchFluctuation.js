  ...
  switch (AreaType)
  {
	...
	case 'Flash':
		 ...
		 else
		 {
		  ...
		  break; 
	case 'Shop':
	     if (SearchShopPower=='False')          //搜索菜品权限检查
		 {
		  DisabledSearchFluctuation(true);
		  return;
		 }
		 else
		 {
		  DisabledSearchFluctuation(false);
	      document.all.DateArea.style.display="";
	      TextArr=new Array('菜品名称','菜品介绍','菜品关键字','厂商','价格')
	      }
		  break; 
    case 'Movie':
   ...
   switch (form.elements[1].value)
    {
	 ...
	 case 'Shop':
		   ...
		   parent.frames['BottomFrame'].location.href='Split.asp?OpStr=商城管理 >> <font color=red>搜索菜品结果</font>&ButtonSymbol=ProductSearch';
		   break;
	  case 'Movie':
